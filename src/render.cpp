...
void ObjectPicker::DoCompare(double distance, int zIndex, int comparePosition) {
    if((fabs(distance - minDistance) < 2.0 && zIndex > minZIndex) ||
       (distance < minDistance - 2.0)) {
...
void ObjectPicker::DoQuad(const Vector &a, const Vector &b, const Vector &c, const Vector &d,
    ...
    DoCompare((insideQuad ? -1.0 : 1.0) * std::min(fabs(minNegative), fabs(maxPositive)),
              zIndex, comparePosition);
}
...
void ObjectPicker::DrawLine(const Vector &a, const Vector &b, hStroke hcs) {
    ...
    DoCompare(distance - stroke->width, stroke->zIndex);
}
...
void ObjectPicker::DrawEdges(const SEdgeList &el, hStroke hcs) {
    ...
    for(const SEdge &e : el.l) {
        ...
        double distance = point.DistanceToLine(ap, bp.Minus(ap), /*asSegment=*/true);
        DoCompare(distance - stroke->width, stroke->zIndex, e.auxB);
    }
}
...
void ObjectPicker::DrawFaces(const SMesh &m, std::unordered_set<uint32_t> faces, hFill hcf) {
    ssassert(false, "Not implemented");
}
...
bool ObjectPicker::Pick(std::function<void()> drawFn, double radius) {
    double oldMinDistance = minDistance;
    drawFn();
    if(minDistance < SELECTION_RADIUS && minDistance < oldMinDistance) {
