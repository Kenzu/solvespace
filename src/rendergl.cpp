//-----------------------------------------------------------------------------
// OpenGL compatibility profile based rendering interface.
//
...
void OpenGL2Renderer::SelectPrimitive(GLenum mode) {
...
void OpenGL2Renderer::UnSelectPrimitive() {
...
void OpenGL2Renderer::DoFatLineEndcap(const Vector &p, const Vector &u, const Vector &v) {
...
void OpenGL2Renderer::DoFatLine(const Vector &a, const Vector &b, double width) {
...
void OpenGL2Renderer::DoPoint(Vector p, double d) {
    if(d <= 3.0) {
...
void OpenGL2Renderer::DrawOutlines(const SOutlineList &ol, hStroke hcs) {
...
void OpenGL2Renderer::DrawPoint(const Vector &o, double s, hFill hcf) {
...
void OpenGL2Renderer::DrawMesh(const SMesh &m, hFill hcff, hFill hbf, hStroke hts) {
    UnSelectPrimitive();

    Fill *frontFill = SelectFill(hcff);
    ssglMaterialRGBA(GL_FRONT, frontFill->color);

    if(hbf.v != 0) {
        Fill *backFill = fills.FindById(hbf);
    ...
    if(hts.v != 0) {
        Stroke *triangleStroke = SelectStroke(hts);
...
void OpenGL2Renderer::DrawFaces(const SMesh &m, std::unordered_set<uint32_t> faces, hFill hcf) {
    SelectFill(hcf);
    SelectPrimitive(GL_TRIANGLES);
    for(const STriangle *tr = m.l.First(); tr; tr = m.l.NextAfter(tr)) {
        if(faces.count(tr->meta.face) == 0) continue;
...
void OpenGL2Renderer::BeginFrame(RgbaColor backgroundColor) {
    ...
    if(EXACT(lightIntensity[0] != 0.0)) {
        glEnable(GL_LIGHT0);
        GLfloat f = (GLfloat)lightIntensity[0];
        GLfloat li0[] = { f, f, f, 1.0f };
        glLightfv(GL_LIGHT0, GL_DIFFUSE, li0);
        glLightfv(GL_LIGHT0, GL_SPECULAR, li0);

        Vector ld = camera.VectorFromProjs(lightDirection[0]);
        GLfloat ld0[4] = { (GLfloat)ld.x, (GLfloat)ld.y, (GLfloat)ld.z, 0 };
        glLightfv(GL_LIGHT0, GL_POSITION, ld0);
    }

    if(EXACT(lightIntensity[1] != 0.0)) {
        glEnable(GL_LIGHT1);
        GLfloat f = (GLfloat)lightIntensity[1];
        GLfloat li0[] = { f, f, f, 1.0f };
        glLightfv(GL_LIGHT1, GL_DIFFUSE, li0);
        glLightfv(GL_LIGHT1, GL_SPECULAR, li0);

        Vector ld = camera.VectorFromProjs(lightDirection[1]);
        GLfloat ld0[4] = { (GLfloat)ld.x, (GLfloat)ld.y, (GLfloat)ld.z, 0 };
        glLightfv(GL_LIGHT1, GL_POSITION, ld0);
    }

    if(EXACT(ambientIntensity != 0.0)) {
        GLfloat ambient[4] = { (float)ambientIntensity,
