...
public:
    ...
    virtual void DrawFaces(const SMesh &m, std::unordered_set<uint32_t> faces, hFill hcf) = 0;
...
public:
    ...
    void DrawFaces(const SMesh &m, std::unordered_set<uint32_t> faces, hFill hcf) override;
    ...
    bool Pick(std::function<void()> drawFn, double radius);
};

// A canvas that uses the core OpenGL profile, for desktop systems.
class OpenGL2Renderer : public Canvas {
public:
    Camera      camera;
    // Lighting parameters.
    ...
    void BeginFrame(RgbaColor backgroundColor);
    void EndFrame();
