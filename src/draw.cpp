...
void GraphicsWindow::Paint() {
    ...
    canvas.ambientIntensity  = SS.ambientIntensity;
    canvas.lightIntensity[0] = SS.lightIntensity[0];
    canvas.lightIntensity[1] = SS.lightIntensity[1];
    canvas.lightDirection[0] = SS.lightDir[0];
    canvas.lightDirection[1] = SS.lightDir[1];
    ...
    if(SS.ActiveGroupsOkay()) {
        ...
        bgColor = Style::Color(Style::DRAW_ERROR);
    ...
    if(SS.showToolbar) {
        ...
        int64_t renderTime = GetMilliseconds() - timestamp;
        RgbaColor renderTimeColor;
        if(1000 / renderTime < 60) {
        ...
        uiCanvas.DrawBitmapText(ssprintf("rendered in %ld ms (%ld 1/s)",
                                         (long)renderTime, (long)(1000/renderTime)),
                                5, 5, renderTimeColor);
    }
